...
class MusicController:
    def __init__(self, bot, guild_id, vc):
        self.bot = bot
...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        if not hasattr(bot, 'wavelink'):
            self.bot.wavelink: wavelink = wavelink
    ...
    async def start_nodes(self):
        ...
        for n in nodes.values():
            node: wavelink.Node = self.bot.wavelink.Node(uri=n['rest_url'], password=n['password'])
            await self.bot.wavelink.NodePool.connect(client=self, nodes=[node])
    ...
    def get_controller(self, value: Union[commands.Context, wavelink.Player], vc):
        ...
        try:
            ...
            controller = MusicController(self.bot, gid, vc)
    ...
    async def connect(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None):
        ...
        # self.bot.wavelink.vc = vc
        setattr(self.bot.wavelink, "vc", vc)

        controller = self.get_controller(ctx, vc)
    ...
    async def play(self, ctx, *, query: str):
        ...
        if not RURL.match(query):
            ...
            tracks = await wavelink.GenericTrack.search(query)

        # tracks = await self.bot.wavelink.Player.get_tracks(query)

        if not tracks:
            return await ctx.send(embed=MusicEmbed(f"**Could not find any songs!`**", ctx.author).get(), delete_after=15)

        if not hasattr(self.bot.wavelink, 'vc'):
            await ctx.invoke(self.connect)
        player = self.bot.wavelink.vc
        if not player.is_connected:
            await ctx.invoke(self.connect)

        controller = self.get_controller(ctx, self.bot.wavelink.vc)
    ...
    async def pause(self, ctx):
        ...
        player = self.bot.wavelink.Player(ctx.guild.id)
    ...
    async def stop(self, ctx):
        ...
        player:wavelink.Player = self.bot.wavelink.vc
